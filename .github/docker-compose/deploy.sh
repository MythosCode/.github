...
display_help() {
    echo "Usage: $0 [CONTAINER_NAME] [PROJECT_NAME] [DOCKER_COMPOSE_FILE]"
    echo
    echo "Positional arguments:"
    echo "  CONTAINER_NAME       Name of the docker container for script to check the status. (Required)"
    echo "  PROJECT_NAME         Name of the project for docker compose to start and stop. (Required)"
...
if [[ -z "$2" ]]; then
    echo "error: PROJECT_NAME is a required argument."
    display_help
    exit 1
fi
